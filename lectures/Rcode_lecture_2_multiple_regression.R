...
# There the original data Car_04.csv is cleaned and a new 
...
identify(data1$Length, data1$MPG_City, labels=data1$Make.Model)
summary(fit1)
# Coefficients:
...
identify(fit1$fitted.values, fit1$residual, labels=data1$Make.Model, pos=3)
...
# 3) A reasonably well fitted line indicates normal errors.
...
# The interpretation is for this new desgin, a 95% CI for its MOG can be 
...
# Coefficients:
#                 Estimate Std. Error t value Pr(>|t|)    
# (Intercept)   38.625874   2.225248  17.358  < 2e-16 ***
#   Length      -0.061913   0.013002  -4.762 3.45e-06 ***
#   Horsepower  -0.036900   0.002772 -13.311  < 2e-16 ***
# 
# Residual standard error: 2.376 on 223 degrees of freedom
# Multiple R-squared:  0.591,	Adjusted R-squared:  0.5873 
# F-statistic: 161.1 on 2 and 223 DF,  p-value: < 2.2e-16  
...
identify(fit2$fitted.values, fit2$residuals, labels = data1$Make.Model)
...
par(mfrow=c(2, 1)) 
plot(fit1, 1, pch=16)  # residual plot for fit1
plot(fit2, 1, pch=15)  # residual plot for fit2
par(mfrow=c(1, 1)) 
...
# 1    224  2258.4                                  
# 2    223  1258.4  1    999.95     177.19  < 2.2e-16 ***

# F_stat=177.19
# p-vavle=pf(177.19, 1, 223, lower.tail=F)
...
#   Length      127.96   1  22.674 3.454e-06 ***
#   Horsepower  999.95   1 177.194 < 2.2e-16 ***
#   Residuals  1258.44 223     
...
# Coefficients:
#               Estimate Std. Error t value Pr(>|t|)    
#   (Intercept)  45.63372    4.02793  11.329  < 2e-16   ***
#   Horsepower   -0.02000    0.00408  -4.902  1.84e-06  ***
#   Length        0.04909    0.01730   2.838  0.00497   ** 
#   Width        -0.35358    0.06878  -5.140  6.07e-07  ***
#   Seating      -0.24135    0.14955  -1.614  0.10799    
#   Cylinders    -0.27169    0.23292  -1.166  0.24471    
#   Displacement -0.93813    0.37166  -2.524  0.01231   *  
#     
...
#   Q1:The coeff for Length is 0.04909. Is this estimate wrong?
...
#         H0: beta_Seating=beta_Cylinder=0
    
    fit3.0=lm(MPG_City~Horsepower+Length+Width+Displacement,
                     data=data1)

    anova(fit3.0, fit3) # At .15 level we reject the null hypothesis.
...
plot(Continent, MPG_City) # box plots
...
#   Coefficients:
#   Estimate Std. Error t value Pr(>|t|)    
#   (Intercept)  18.7297     0.4197  44.624  < 2e-16 ***
#   ContinentAs   1.5152     0.5560   2.725  0.00694 ** 
#   ContinentE   -0.4705     0.6462  -0.728  0.46734    
# ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Residual standard error: 3.611 on 223 degrees of freedom
# Multiple R-squared:  0.05522,	Adjusted R-squared:  0.04674 
# F-statistic: 6.517 on 2 and 223 DF,  p-value: 0.001776

# 1) The intercept beta_0=mean(MPG|Am) is the mean for the base Am
#  # same as mean(data1$MPG_City[data1$Continent=="Am"])
...
#   Coefficients:
#   Estimate Std. Error t value Pr(>|t|)    
#   (Intercept)  20.2449     0.3647  55.508  < 2e-16 ***
#   ContinentAm  -1.5152     0.5560  -2.725  0.00694 ** 
#   ContinentE   -1.9856     0.6119  -3.245  0.00136 ** 
#   ---
#   Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
# 
# Residual standard error: 3.611 on 223 degrees of freedom
...
Fstat
pvalue=1-pf(Fstat[1], 6, 219) 
pvalue #or
pf(Fstat[1], 6, 219, lower.tail=FALSE)

qf(.95, 6, 219) # As long as Fstat is larger than 2.14, we reject the null at .05 level.
